...
@experiment_start(name="portfolio", inputs=["tickers"],
                  ...
                  trace_link=lambda ret: ret[1],            # --trace links each case to its span
                  evaluators=_portfolio_evaluators)         # richer checks (Step 3)
async def analyze_portfolio(tickers): ...
```

It's a pure no-op in normal execution — it only activates under `ddtrace-experiment`.
...
> `capture`/`replay` run the **real, paid** agent (minutes per case) — `WATCHLISTS="NVDA"` keeps it to one.
...
```bash
exp capture capture_watchlists:generate_traffic   # 1. baseline current behavior -> .llmobs_experiments.json
exp list   capture_watchlists                     #    confirm the boundary registered -> portfolio
# 2. change a model / prompt / tool / schema
exp replay src.agents.orchestrator                # 3. structural check (default) -> match / changed
exp replay src.agents.orchestrator --evaluate     # 3b. + completeness / sentiment / grounding
exp replay src.agents.orchestrator --publish --project stock-watchlist-agent   # 4. UI + eval metrics
```
...
`replay` exits non-zero on `changed`, `ERROR`/`NO_END`, or (with `--evaluate`) any evaluator
...
| Add a `risk_rating` field (intentional)                | `changed` everywhere — expected; re-run `capture` to reset the baseline                       |
...
- **Re-baseline after intended changes** — an accepted change _should_ report `changed`; re-run `capture`.
- **Cost** — every `capture`/`replay` runs the real agent; `--evaluate` adds judge calls. Keep `WATCHLISTS` small.
- **`--publish` writes to the backend** (Dataset + Experiment; needs `DD_API_KEY`); the LLM judges also call OpenAI.
- **`--trace` / full-trace export** requires a recent `ddtrace` (older versions can fail to serialize some provider-specific span metadata).
